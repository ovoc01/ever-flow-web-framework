...
public class FrontServlet extends HttpServlet {
    ...
    private void retrieveAllMappedMethod(String paths) throws  IOException {
        ...
        for (Class clazz : classes) {
            ...
            for (Method method : methods) {
                ...
                if (link != null) {
                    mappingUrl.put(link.url(), new Mapping(clazz.getName(), method, clazz));
                }
            }
        }
    }
    ...
    public final void processRequest(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        try {
            ...
            Object requestResponse = redirection(request,response);
    ...
    private void dispatchNormal(ModelView modelView) {
        if (modelView.getData().size() > 0)
            addDataToRequest(modelView.getData()); //manampy attribut ao anatin'ilay httpServletRequest avy any @ ilay data ao anaty modelView
        if (modelView.getSessions().size() > 0)
            addDataToSession(modelView.getSessions()); //manampy session ao anatin'ilay HttpSession avy any @ ilay session anaty modelView
        if (modelView.getRemoveSession().size() > 0)
            removeSpecificSession((modelView.getRemoveSession())); // manala session iray na maro maro precis ao anatin'ilay HttpSession
    ...
    private Object redirection(HttpServletRequest request,HttpServletResponse response) throws InstantiationException, IllegalAccessException, NoSuchMethodException, InvocationTargetException, ServletException, IOException {
        ...
        if (!links.isEmpty()) {
            ...
            if(links.get(0).equals("download.do")){
                String path = (String) request.getParameter("img");
                download(uploadFolder+"/"+path,response);
            ...
            if (objectMapping != null) {
                ...
                throw new InstantiationException("Url not supported");
            }
    ...
    private void setSessionInObject(Object object, HttpSession httpSession) throws RuntimeException, IllegalAccessException {
        ...
        if (session == null)
            throw new RuntimeException("To access Session with @Session annotation you should add a field  HashMap<String,Object> called session");
    ...
    private void sessionSessionInObject(Object object, Field field, HttpSession httpSession) throws IllegalAccessException {
    ...
    private void methodAuthentification(Method method, HttpSession session) throws RuntimeException {
        ...
        if (session.getAttribute(getProfileName()) == null || !session.getAttribute(getProfileName()).equals(authentification.auth()))
    ...
    private Object[] dynamicCast(Class<?>[] classes, Object[] args) {
        ...
        for (Class<?> cl : classes) {
            if (cl == Integer.class) array[i] = Integer.parseInt(String.valueOf(args[i]));
            else if (cl == Timestamp.class) array[i] = Timestamp.valueOf(String.valueOf(args[i]));
            else if (cl == Double.class) array[i] = Double.parseDouble(String.valueOf(args[i]));
            else if (cl == Float.class) array[i] = Float.parseFloat(String.valueOf(args[i]));
            else array[i] = (String.valueOf(args[i]));
            i++;
        }
        return array;
    }
    ...
    private Vector<Parameter> annotedMethodParameters(Method method) {
        ...
        for (Parameter params : method.getParameters()) {
            if (params.isAnnotationPresent(RequestParameter.class)) parameterVector.add(params);
        }
